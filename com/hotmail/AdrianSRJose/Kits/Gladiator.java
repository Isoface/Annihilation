...
public class Gladiator extends ClassItemKit {
...
//	private static Material TORCH = Material.TORCH;
	...
	protected ItemStack specialItem() {
		ItemStack stack = new ItemStack ( UniversalMaterial.TORCH.getMaterial ( ) );
		...
		for (Entity nearbysOfNexus : KitConfig.getNearbyEntities(ap.getTeam().getNexus().getLocation().toLocation(), 8,
			...
			if (nearbysOfNexus != null && nearbysOfNexus instanceof Player) {
				...
				if (nearbyAp != null && !nearbyAp.getTeam().equals(ap.getTeam())) {
					...
					//
					Object cs = nearbyAp.getData("key-gladiator-helmet-backup-key");
					if (cs == null && nearby.getInventory() != null) {
						ItemStack h = nearby.getInventory().getHelmet();
						if (h != null)
							nearbyAp.setData("key-gladiator-helmet-backup-key", h);
						//
						nearby.getInventory().setHelmet(new ItemStack(UniversalMaterial.PACKED_ICE.getMaterial(), 1));
						//
						Bukkit.getScheduler().runTaskLater(AnnihilationMain.INSTANCE, new Runnable() {
							@Override
							public void run() {
								if (nearbyAp != null && nearbyAp.isOnline()) {
									Object o = nearbyAp.getData("key-gladiator-helmet-backup-key");
					...
					nearby.getLocation().getWorld().playSound(nearby.getLocation(),
							UniversalSound.ZOMBIE_UNFECT.asBukkit(), 2.0F, 6.0F);
					//
					//
...
//								+ ChatColor.YELLOW + " enemies on your Nexus"));
	...
	protected ItemStack getDefaultIcon() {
		return new ItemStack ( UniversalMaterial.TORCH.getMaterial ( ) );
	}
	...
	public boolean onItemClick(Inventory paramInventory, AnniPlayer ap) {
		...
		if (ap.getPlayer().getInventory().getBoots() == null
			...
			ap.getPlayer().getInventory().setBoots(KitUtils.addSoulbound(new ItemStack ( UniversalMaterial.CHAINMAIL_BOOTS.getMaterial ( ) , 1, (short) 96)));
		}
		else {
			paramInventory.addItem(KitUtils.addSoulbound(new ItemStack( UniversalMaterial.CHAINMAIL_BOOTS.getMaterial ( ) , 1, (short) 96)));
		}
		...
		if (ap.getPlayer().getInventory().getChestplate() == null
			...
			ap.getPlayer().getInventory().setChestplate(KitUtils.addSoulbound(new ItemStack ( UniversalMaterial.CHAINMAIL_CHESTPLATE.getMaterial ( ) )));
		}
		else {
			paramInventory.addItem(KitUtils.addSoulbound(new ItemStack( UniversalMaterial.CHAINMAIL_CHESTPLATE.getMaterial ( ) )));
		}
