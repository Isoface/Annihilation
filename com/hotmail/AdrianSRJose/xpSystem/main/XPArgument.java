...
public class XPArgument implements AnniArgument {
		...
		if (args != null && args.length > 2) {
			try {
				if (args[0].equalsIgnoreCase("add") || args[0].equalsIgnoreCase("subtract")) {
					...
					sender.sendMessage("You did not specify a valid XP operation.");
	...
	public String getHelp() {
		return ChatColor.LIGHT_PURPLE + "XP [add,substract] <amount> <player>--" + ChatColor.GREEN
				+ "adds or subtracts XP from a from player.";
	}
