...
public class NPCPath {
	...
	public boolean update() {
		...
		if (d + speed < 1) {
			...
			entity.move(EnumMoveType.SELF, dx, 0, dz);
			...
			if (!nmsPath.b()) {
				...
				double dy = currentPoint.y - entity.locY; // Jump if needed to reach next block.
				double dz = bz + ((currentPoint.z - entity.locZ) * d2);

				entity.move(EnumMoveType.SELF, dx, dy, dz);
				entity.checkMovement(dx, dy, dz);
				progress += speed;
			} else {
				// Complete final movement
				entity.move(EnumMoveType.SELF, bx, 0, bz);
	...
	public static NPCPath find(NPCEntity entity, Location to, double range, double speed) {
		...
		// int k = (int) (range + 8.0);
		//
		// ChunkCache chunkCache = new ChunkCache(entity.world, posFrom.a(-k, -k, -k),
		// posFrom.a(k, k, k), 0);
		// PathEntity path = entity.getPathfinder().a(chunkCache, entity, posTo, (float)
		// range);
		// if(path != null) {
