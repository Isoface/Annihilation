...
import com.hotmail.AdrianSRJose.AnniPro.utils.ReflectionUtils.PackageType;
...
public enum OldParticleEffect {
	 ...
	 */
	private OldParticleEffect(String name, int id, int requiredVersion, ParticleProperty... properties) {
		this.name = name;
		this.id = id;
	...
	public static OldParticleEffect fromName(String name) {
		for (Entry<String, OldParticleEffect> entry : NAME_MAP.entrySet()) {
	...
	public static OldParticleEffect fromId(int id) {
		for (Entry<Integer, OldParticleEffect> entry : ID_MAP.entrySet()) {
			if (entry.getKey() != id) {
				continue;
			}
			return entry.getValue();
	 ...
	 */
	private static boolean isDataCorrect(OldParticleEffect effect, ParticleData data) {
	 ...
	 */
	private static boolean isColorCorrect(OldParticleEffect effect, ParticleColor color) {
	 ...
	 * @see ParticlePacket#ParticlePacket(OldParticleEffect, Vector, float, boolean,
	 *      ParticleData)
	 * @see ParticlePacket#sendTo(Location, double)
	 */
	 ...
	 *             incorrect
	 * @see ParticlePacket#ParticlePacket(OldParticleEffect, ParticleColor, boolean)
	 * @see ParticlePacket#sendTo(Location, double)
	 */
	 ...
	 * {@link OldParticleEffect#ITEM_CRACK}, {@link OldParticleEffect#BLOCK_CRACK} and
	 * {@link OldParticleEffect#BLOCK_DUST}
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same
	 * usage conditions
	 * 
	...
	/**
	 * Represents the item data for the {@link OldParticleEffect#ITEM_CRACK} effect
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same
	 * usage conditions
	 * 
	...
	/**
	 * Represents the block data for the {@link OldParticleEffect#BLOCK_CRACK} and
	 * {@link OldParticleEffect#BLOCK_DUST} effects
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same
	 * usage conditions
	 * 
	...
	/**
	 * Represents the color for effects like {@link OldParticleEffect#SPELL_MOB},
	 * {@link OldParticleEffect#SPELL_MOB_AMBIENT}, {@link OldParticleEffect#REDSTONE} and
	 * {@link OldParticleEffect#NOTE}
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same
	 * usage conditions
	 * 
	...
	/**
	 * Represents the color for effects like {@link OldParticleEffect#SPELL_MOB},
	 * {@link OldParticleEffect#SPELL_MOB_AMBIENT} and {@link OldParticleEffect#NOTE}
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same
	 * usage conditions
	 * 
	...
	/**
	 * Represents the color for the {@link OldParticleEffect#NOTE} effect
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same
	 * usage conditions
	 * 
	...
	public static final class ParticlePacket {
		...
		private final OldParticleEffect effect;
		 ...
		 * @see #initialize()
		 */
		public ParticlePacket(OldParticleEffect effect, float offsetX, float offsetY, float offsetZ, float speed,
		 ...
		 */
		public ParticlePacket(OldParticleEffect effect, Vector direction, float speed, boolean longDistance,
		 ...
		 *      ParticleData)
		 */
		public ParticlePacket(OldParticleEffect effect, ParticleColor color, boolean longDistance) {
			this(effect, color.getValueX(), color.getValueY(), color.getValueZ(), 1, 0, longDistance, null);
			if (effect == OldParticleEffect.REDSTONE && color instanceof OrdinaryColor
			...
			try {
				...
				if (version < 8) {
					...
					if (data != null) {
						...
						ReflectionUtils.setValue(packet, true, "k", effect == OldParticleEffect.ITEM_CRACK ? packetData
		...
		public void customSendTo(Location center, double range, ParticleData check) throws IllegalArgumentException {
			if (range < 1) {
				throw new IllegalArgumentException("The range is lower than 1");
			}

			if (!AnnihilationMain.VIA_VERSION) {
				sendTo(center, range);
				return;
			}
			...
			if (center != null) {
				if (center.getWorld() != null) {
					...
					for (Player player : Bukkit.getOnlinePlayers()) {
						...
						if (AnnihilationMain.versionHandler.getVersion(player) > 47) {
							sendTo(center, player);
						} else {
							data = check;
							sendTo(center, player);
